...
def extract_role_and_company(job_desc):
    ...
    try:
        ...
        logger.warning("Error extracting role/company from resume JD: %s", safe_log_text(e))
        return "Unknown Role", "General"

def analyze_resume_job_description(resume_text, job_desc, extra_sections=None):
    """
    Analyzes the resume specifically against a provided Job Description.
    """
    ...
    resume_block = (
        f"--- BEGIN PDF EXTRACTED RESUME ---\n"
        f"{resume_text}\n"
        f"--- END PDF EXTRACTED RESUME ---"
    )

    prompt = f"""
<system_instructions>
You are an expert Career Coach, ATS Specialist, and LinkedIn Profile Optimizer.
Your goal is to transform a user's Resume into a top-tier, 100/100 scored LinkedIn profile that perfectly aligns with a specific Job Description (JD).
You must strictly follow the output format and structural requirements.
</system_instructions>

<context>
    <target_role>{extracted_role}</target_role>
    <target_company>{extracted_company}</target_company>
    <job_description>
    {job_desc}
    </job_description>
    <company_guidelines>
...
<task_description>
1.  **Analyze**: Deeply analyze the user's Resume against the provided Job Description. Identify gaps, weak language, and missing keywords from the JD.
2.  **Strategize**: Determine the best angle to position this candidate. How can their past experience be framed to look perfect for this specific JD?
3.  **Transform**: Convert the Resume content into a high-quality LinkedIn profile to achieve a **100/100 score**.
    *   **Score Criteria**:
...
"""
    ...
    log_data = {
        'type': 'resume_job_description_analysis',
        'inputs': {
